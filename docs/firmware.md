...
**emonTx5_DB_6CT_1phase_LPL**<br>
Use this firmware for single phase electricity monitoring with up to 6 CT sensors. Pulse sensing is configured on the emonTx5 analog input port which can be configured using a solder bridge. Radio data is transmitted to a receiving emonPi/emonBase via RFM69CW radio. Voltage & CT sensor calibration, pulse sensor configuration and radio settings can be applied via serial configuration.

Note: This firmware **does not support temperature sensing**, please use the `emonTx5_CM_6CT_temperature_LPL` firmware if you wish to use temperature sensors.

This firmware uses the emonLibDB library and is compiled from the `emon_DB_6CT` base firmware.

**emonTx5_DB_6CT_3phase_LPL**:<br>
...
Note: This firmware **does not support temperature sensing**, temperature sensing is only available via the single-phase `emonTx5_CM_6CT_temperature_LPL` firmware.
...
The standard radio format is 'LowPowerLabs', if you wish to use the emonTx5 with an original emonPi (v1) system running JeeLib classic radio format you can select the JeeLib classic radio format here.
...
3. emonTx5_CM_6CT_temperature_LPL          1.6.0      (Standard LowPowerLabs)
```
